...
const modules = [
    ...
    // { name: "Weekly Review", id: "weekly-review", path:"/weekly-review", icon: <NotebookPen size={18} />, desc: "Reflect weekly" },
...
const Sidebar = () => {
    ...
    return (
        <aside
            className={`h-screen border-r shadow-lg p-4 transition-all duration-500
    ${collapsed ? "w-20" : "w-64"}
    bg-[var(--bg)] text-[var(--text)] border-gray-200 dark:border-gray-700`}
        >
            ...
            <nav className="space-y-2">
                {modules.map((mod) => (
                  ...
                  >
                        {mod.icon}
                        {!collapsed && <span
                            className={`text-sm font-medium sidebar-text ${collapsed ? "sidebar-collapsed" : ""
                                }`}
                        >
                            {mod.name}
                        </span>}
